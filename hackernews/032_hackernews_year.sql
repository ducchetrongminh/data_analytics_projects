WITH summary_hackernews_user AS (
  SELECT 
    DATE_TRUNC(first_posted_date, YEAR) AS year
...
, calculate_metrics_1 AS (
  SELECT 
    *
    , SUM(new_users) OVER (ORDER BY year ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS total_users_start_of_year
    , SUM(new_users) OVER (ORDER BY year) AS total_users_end_of_year
